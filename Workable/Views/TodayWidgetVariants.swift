...
struct TodayWidgetData {
    ...
    static let mock = TodayWidgetData(
        ...
        anomalyPills: {
            ...
            return [
                TodayAnomalyPill(label: "No clock in",           count: eligible.filter { $0.anomalyType == .noClockIn }.count,            style: .danger,  filters: [.noClockIn]),
                TodayAnomalyPill(label: "No clock in nor out",   count: eligible.filter { $0.anomalyType == .noClockInNorOut }.count,      style: .danger,  filters: [.noClockInNorOut]),
                TodayAnomalyPill(label: "Exceeds work schedule", count: eligible.filter { $0.anomalyType == .exceededWorkSchedule }.count, style: .warning, filters: [.exceededWorkSchedule]),
                TodayAnomalyPill(label: "On track",              count: eligible.filter { $0.anomalyType == .onTrack }.count,               style: .success, filters: [.onTrack]),
                TodayAnomalyPill(label: "All",                   count: eligible.count,                                                     style: .neutral, filters: []),
            ]
        }(),
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            OnLeaveRowView(
                ...
                avatarNames: data.onLeaveAvatars,
                overflowCount: data.onLeaveOverflow,
                showChevron: false
            )
...
struct TodayWidgetCompact: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 16) {
                ForEach(data.anomalyPills.filter { $0.label != "All" }) { pill in
                    HStack(spacing: 4) {
                        Circle()
            ...
            LazyVGrid(columns: gridColumns, spacing: 12) {
                gridCard {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        let total = data.anomalyPills.first(where: { $0.label == "All" })?.count ?? 0
                        Text("\(total)")
...
struct TodayWidgetRich: View {
    ...
    private var anomalyBarChart: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            let maxCount = data.anomalyPills.filter { $0.label != "All" }.map(\.count).max() ?? 1
            ForEach(data.anomalyPills.filter { $0.label != "All" }) { pill in
                HStack(spacing: 8) {
                    Text(pill.label)
                        .font(AppFonts.caption1())
...
struct TodayWidgetTimeline: View {
    ...
    private var timelineNodes: [TimelineNode] {
        ...
        if dangerCount > 0 {
            nodes.append(TimelineNode(time: "—", title: "\(dangerCount) anomalies detected", subtitle: "Missed clocks & attendance issues", kind: .anomaly))
        }
...
// MARK: - Equatable for AnomalyPillStyle (needed for filter)

extension AnomalyPillStyle: Equatable {}
