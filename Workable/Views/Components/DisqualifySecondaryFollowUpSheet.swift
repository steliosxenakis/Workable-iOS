...
/// Secondary follow-up in disqualify flow (triggered by "Disqualify candidate only").
struct DisqualifySecondaryFollowUpSheet: View {
    ...
    private var explanation: String {
        "Disqualifying \(candidateFirstName) now will stop all current AI actions and end any active chat invitation."
    }
    ...
    var body: some View {
        NavigationStack {
            GeometryReader { geo in
                ScrollView {
                    VStack(spacing: 0) {
                        ...
                        Text("Disqualify without take over?")
            ...
            .sheet(isPresented: $showFinalConfirmation) {
                DisqualifyFinalConfirmationSheet(candidate: candidate) {
                    onConfirm()
                    dismiss()
                }
            ...
            VStack(spacing: 12) {
                ...
                Button {
                    ...
                    Text("Continue")
                        ...
                        .multilineTextAlignment(.center)
                        .frame(maxWidth: .infinity, minHeight: 54)
                        .background(AppColors.dangerDefault)
                        .clipShape(Capsule())
...
#Preview {
    DisqualifySecondaryFollowUpSheet(
        candidate: Candidate(
            name: "Rachael",
            role: "Designer",
