...
/// Secondary follow-up in send text flow.
struct SendTextSecondaryFollowUpSheet: View {
    ...
    private var explanation: String {
        "If you continue, the AI agent invitation and related actions will stop for \(candidateFirstName)."
    }
    ...
    var body: some View {
        NavigationStack {
            GeometryReader { geo in
                ScrollView {
                    VStack(spacing: 0) {
                        ...
                        Text("Send text without take over?")
            ...
            .sheet(isPresented: $showFinalConfirmation) {
                SendTextFinalConfirmationSheet(candidate: candidate) {
                    onConfirm()
                    dismiss()
                }
...
#Preview {
    SendTextSecondaryFollowUpSheet(
        candidate: Candidate(
            name: "Rachael",
            role: "Designer",
